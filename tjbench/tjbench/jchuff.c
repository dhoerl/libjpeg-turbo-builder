 ...
 */
/* if (__builtin_expect(c == 0xFF, 0)) DFH */
...
#define xCHECKBUF31() { \
  if (put_bits > 31) { \
	size_t o_put_buffer = put_buffer; \
	total++; \
    xEMIT_BYTE(0) \
    xEMIT_BYTE(1) \
    xEMIT_BYTE(2) \
...
encode_one_block (working_state * state, JCOEFPTR block, int last_dc_val,
                  c_derived_tbl *restrict dctbl, c_derived_tbl *restrict actbl)
{
  int temp, temp2, temp3;
  int nbits;
  int r, code, size;
  JOCTET _buffer[BUFSIZE], *restrict buffer;
  size_t put_buffer;  int put_bits;
...
//printf("put_bits %d\n", put_bits);
	...
	if(!fp) {
		fp = fopen("/tmp/totals", "w+");
	}
	fprintf(fp, "TOTAL: %d HITS %d\n", total, hTotal);
	fflush(fp);
}
*/
